    ...
    return (
        <>
            {comments.length > 0 ? comments.map((dataComment, index) => (
                <div className="bg-[#1f1f20] w-[500px] p-3 rounded-[10px] my-2" key={index}>
                    <div className="flex justify-between">
                        <div className="flex gap-2">
                            <img className="w-[30px] rounded-full" src={dataComment.user.avatar ? dataComment.user.email : avatar} alt="" />
                            <span className="text-[#fff]">{dataComment.user.email} {dataComment.user.admin && '( Admin )'}</span>
                        </div>
                        <div>
